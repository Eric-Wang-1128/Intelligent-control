...
for k = 1:K
    ...
    % x(1, :) = zeros(1,length(time)+1);
    % x(2, :) = zeros(1,length(time)+1);
    x(:, 1) = x0;
    ...
    % e_1 = x(1, :) - xd(1, :);
    % e_2 = x(2, :) - xd(2, :);
    % e_ref_t = @(t) ((e_1(1)+e_2(1)*t) * (0.3 * (1 - t / Td).^5 - 1.5 * (1 - t / Td).^4 + 1.0 * (1 - t / Td).^3));
    % e1_star_t = @(t) ((e_1(1)+e_2(1)*t) .* (0.3 * (1 - t / Td).^5 - 1.5 * (1 - t / Td).^4 + 1.0 * (1 - t / Td).^3)) .* (t <= Td);
    % e1_star = e1_star_t(t);
    % 
    % f_j = polyfit(T, e1_star, 2);  % 擬合二次多項式
    % p_prime = polyder(f_j);  % 對多項式求導
    % e2_star = differentiate(@(t)e1_star_t(t),t,dt);
    % 
    % f_j_2 = polyfit(T, e2_star, 2);
    % p_prime_2 = polyder(f_j_2);
    % e2_star_diff = polyval(p_prime_2, T);
    % 
    % z_1 = e_1 - e1_star;
    % z_2 = e_2 - e2_star;
    % z = [z_1;z_2];
    % 
    % s = [c_1,1] * z;
    % x_bar = [x(1, :);x(2, :);xd(1,:);xd(2,:);xd(3,:);e1_star;e2_star;e2_star_diff];
    e_10 = x(1, 1) - xd(1, 1);
    e_20 = x(2, 1) - xd(2, 1);
    ...
    for t = 1:length(T)
        ...
        e_1 = x(1, t) - xd(1, t);
        e_2 = x(2, t) - xd(2, t);
        %e1_star_t = @(tau) ((e_10+e_20*tau) * (6 * (1 - tau / Td)^5 - 15 * (1 - tau / Td)^4 + 10 * (1 - tau / Td)^3)) .* (t*dt <= Td);
        %e1_star_t
        syms tau 
        ...
        %e1_star = e1_star_t(t*dt);
        %@(tau) differentiate(@(tau) er_func(e10, e20, tau, Td), tau, dt);
        %e2_star_t = diff(e1_star_t, tau);
        %e2_star_t
        %e2_star_t = @(t) differentiate(e1_star_t, t, dt);
        %e2_star_t
        %e2_star = e2_star_t(t*dt);
        %f_double_prime = @(tau) differentiate(f_prime, tau, dt);
        % e2_star_t_diff = diff(e2_star_t, tau);
        % e2_star_t_diff
        %e2_star_t_diff = @(t) differentiate(e2_star_t, t, dt);
        %e2_star_t_diff
        %e2_star_diff = e2_star_t_diff(t*dt);

        % e1_star = subs(e1_star_t, [tau, e10, e20, Td], [t, 0.05, 0.05, 0.4]);
        % e2_star = subs(e2_star_t, [tau, e10, e20, Td], [t, 0.05, 0.05, 0.4]);
        % e2_star_diff = subs(e2_star_t_diff, [tau, e10, e20, Td], [t, 0.05, 0.05, 0.4]);
        e1_store(t,k) = e_1;
        e2_store(t,k) = e_2;
        ...
        % f_j_2 = polyfit(T, e2_star, 2);
        % p_prime_2 = polyder(f_j_2);
        %e2_star_diff = differentiate(@(t)e2_star(t),t,dt);
        ...
        if k == 1
            ...
            %w_hat(:,:,k) = zeros(length(time),13);
            w_hat_diff(t,:) = (-gamma_1/1-gamma_1) * w_hat(t,:,k) + (beta1 * rho * s) * psi(t,:) / (1-gamma_1);

            % for i = 1:size(w_hat_diff,2)
            %     if w_hat_diff(t,i) > 100000
            % 
            %         w_hat_diff(t,i) = 100000;
            %     end
            % end
        ...
        if k == 1
            ...
            % if delta_hat_diff(t,k) > 100000
            % 
            %     delta_hat_diff(t,k) = 100000;
            % end
    ...
    total_speed_error_1 = sqrt(mean(e_1(1,:).^2));
    error_1(k) = total_speed_error_1;
    total_speed_error_2 = sqrt(mean(e_2(1,:).^2));
    error_2(k) = total_speed_error_2;
end
...
plot(time, xd_store(1,:,i), 'b--', 'LineWidth', 1.5);hold on;
plot(time, xd_store(1,:,i) + kc_upper, 'm--', 'LineWidth', 1.5);hold on;
plot(time, xd_store(1,:,i) - kc_lower, 'm--', 'LineWidth', 1.5);hold on;
...
plot(time, xd_store(2,:,i), 'b--', 'LineWidth', 1.5);hold on;
...
title('error Trajectories vs Reference of Joint Angle');

subplot(2,2,4)
plot(time, e2_store(:,i), 'r', 'LineWidth', 1.5); hold on;
title('error Trajectories vs Reference of Angle speed');
end

figure;

subplot(2,1,1)
plot(1:K, error_1, 'r', 'LineWidth', 1.5);
title('RMS error of Joint Angle');

subplot(2,1,2)
plot(1:K, error_2, 'r', 'LineWidth', 1.5);
title('RMS error of Angle speed');
...
% title('Tracking error Trajectories')
% 
% subplot(3, 2, 4);
